...
Tコースのレッスンでは、標準的なキーボードのタッチタイピングを学習します。
キーボード上の各文字を各レッスンで少しずつ紹介してゆきますが、このコース
を終わった時には、アルファベット、数字、大多数の句読点をタッチタイプで
入力できるようになっていることでしょう。
...
あなたがすでにタッチタイピングで入力できるのならば、これからのレッスンは
手早く終わりにできるはずです。直接 Sコースに進んでもかまいません。\T
...
薬指はS、Lのキーのためにそれぞれ使います。\I
sss lll sss lll sss ss ll ss ll s l\D
小指はA、;のキーのためにそれぞれ使います。\I
aaa ;;; aaa ;;; aa ;; aa ;; a ;\D
今日はこれで十分です。復習してみましょう。\I
...
このレッスンでは、すでに学んだ文字の復習と、新しい文字 E、G、H およびカ
ンマ「,」（コンマ）の練習をします。

これらの文字のために指を伸ばしても、必ずホームポジションに戻るようにして
下さい。例えば、Hのキーを打つためには右の人差し指を少し左に動かしますが、
Hのキーを押したらすぐにJの上に指を戻します。
...
左手が担当する文字の大文字を入力したい場合には、右側のシフトキーを使用し
なければなりません。これは以下のようにします:

	a. 右人差し指はJから動かすことなく、小指をシフトキーの上まで伸ば
	   してそれを押します。
...
右手側にある文字を大文字にしてみましょう:\I
Joe Kip Leif Peg Olaf Ira Heidi\D
左手側の文字を大文字にするには、右のシフトキーを使いましょう:\I
...
コロン「:」は「;」の位置（あるいは隣）にあります。
いずれにせよ、右の小指でタイプします。\I
;;; ;:; ;;; ::: ;;; ;:; ;;;\D
コロンの後には常に２つの空白を置きます。\I
...
複数の行にまたがる文の入力では、それぞれの行末で改行(Return)を打ちます。\I
...
ハイフンには３つの使い方があります。まず、(1)単語の結合です:\I
...
*T10*
			     Lesson T10\B
数字キーの練習の前に、アルファベット全部のおさらいです。\I
abcd efg hijk lmnop qrs tuv wx yz\D
もう一度。\I
abcdefghijklmnopqrstuvwxyz\D
それぞれにシフトキーを使って下さい(CAPS-LOCKはダメ):\I
ABCDEFGHIJKLMNOPQRSTUVWXYZ\D
では、句読点のおさらい:\I
l;l l:l k,k l.l ;/; ;?; :?: ;-;\D
...
端末には２種類の数字キーがあります。１つはタイプライタキーボードの一部と
して、アルファベットのキーのすぐ上にあるもの。もうひとつはキーボードの右
の、いわゆるテンキー（電卓キー）です。
このコースでは、テンキーでなく、キーボードの上一列の数字キーだけを扱うこ
とにします。
（テンキーのタッチタイプを練習するには、Nコースを選択して下さい。ただし、
すべてのキーボードにテンキーが付いているわけではありません。）

機種によっては特別な配列のものもありますが、ここでは qwert...の上に
1234567890が一列に並んでいるものを考えます。\T
1からはじめよう！（Aの指を使います）\I
...
レッスン13というのはありません。13が不吉な数字、というのは知っています？

レッスンT1からT12まで終了したら、あなたはもう立派な「タッチタイピスト」
です。標準化されているすべてのキーを学んだことになります。
さらに腕を上げるために、スピード・ドリル（Sコース）に挑戦するのも良いで
しょう。（まだ学んでいない記号に出会ったら、単純に目でそれを捜せば良い
でしょう。右小指をシフトキーにのせ、左の適当な指で数字キーの上の記号を
入力するのです。）

では、キーボードの上の残された文字のレッスンに移りましょう。
#if e
ただし、以下では 101キーボードに準拠して説明します。\T
#else
ただし、以下では 106キーボードに準拠して説明します。\T
#endif
*
*T14*
			    Lesson T14\B
#if e
アポストロフィ「'」(引用符)には「;」の指を使います。\I
;;; ;'; ;;; ''' ;;; ;'; ;;;\D
;'; it's ;'; 'till ;'; won't ;';
;'; 'tis ;'; isn't ;'; 'tis too ;';\D
#else
アポストロフィ「'」(引用符)には「J」の指を使います。\I
...
#if e
ダブルクォート「"」(二重引用符)はシフトキーと「'」で打ちます。\I
;;; ;"; ;;; """ ;;; ;"; ;;;\D
;"; "." ;"; ":" ;"; "," ;";
#else
ダブルクォート「"」(二重引用符)はシフトキーと「2」で打ちます。\I
...
このレッスンでは、プログラミング言語で必要となるような記号を学びます。が、
ここではあまり沢山練習しなくてもいいでしょう。（多分、プログラムの入力で
山ほど練習できるでしょう...）\T
「=」には「;」の指を使います。\I
;;; ;=; ;;; ;=; ;;; ;=; ;;;\D
;=; -=- ;=; '=' ;=; :=: ;=;
...
k<k <.> k<k >.< k<k <<< k<k\D
以下の BASICプログラムを打ってみましょう。\I
10 LET x=10
20 LET y=20
30 PRINT x+y/3
40 PRINT y/x*5
50 STOP\P
CAPS LOCK（あるいはアルファロック）を押して、入力してみましょう:\I
...
このレッスンでは、シフトキーと数字キーで入力する記号について練習します。
これらは文章ではそう頻繁には利用されないので、ここでも練習の分量はあまり
多くありません。

これらの特別な記号を含む文書を打っていて、そのキーの位置が分からなかった
ら、キーを目で捜してもかまいません。しかし、そのキーはつねに正しい指でタ
イプするようにします。
もし、そういうキーが頻繁に出て来るのであれば、それがどこにあるのか、見な
くても自然に覚えられるでしょう。
これらの文字はあまり用いられず、また、キーボードによっても位置が異なるの
で、スピードは重要ではありません。\T
#if e
「@」にはSの指を使います。\I
...
これでTコースは終わりです。この後は、さらにNコース、Sコースを練習すること
ができるでしょう。

まだ学んでいないキーが少々残っています。これらをタッチタイピングで入力しよ
うとするならば、そのキーに一番近い指を使うことです。従って、ESC、TABなどは
Aの指を使います。右の方にある、Delete、Return、「{」、「}」などは「;」の指
を使います。

Controlキーとの組み合わせはタッチタイプで入力するのは難しいですが、可能な
範囲でシフトキーと同様に扱うことはできそうです。しかし、例えばControl-Dは
この方法では難しいでしょう。多くの人は小指で Control を押し、他の指、例え
ば人差し指でDを押す、というようにしているようです。
